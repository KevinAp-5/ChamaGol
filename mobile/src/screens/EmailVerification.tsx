...
const EmailVerificationScreen = ({ navigation }: any) => {
  ...
  const fetchApi = async() => {
    ...
    try {
      ...
      if (response.status != 200) {
        showAlert("Erro ao reenviar e-mail, tente novamente.", { title: "Erro" });
        setEmailSent(true);
        return;
      }
      showAlert("Email enviado com sucesso! Verifique a caixa de entrada", { title: "Sucesso!" });
    } catch (error) {
      showAlert("Erro ao reenviar e-mail", { title: "Erro" });
    }
  }
